...
import { useEffect, useState, type ChangeEvent, type FormEvent, use } from "react";
...
export default function EditBlogPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const loadBlog = async () => {
      try {
        ...
        if (!res.ok) { setError("Blog not found."); setLoading(false); return; }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => null);
        setError(d?.error ? JSON.stringify(d.error) : "Failed to update blog.");
        return;
      }
  ...
  if (loading) {
    return <div className="text-gray-400 text-sm py-12 text-center">Loading blog…</div>;
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Update blog article details</p>
        </div>
